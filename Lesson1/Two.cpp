...
void Run() {
  do {
    ...
    auto interest_rate = RequestInput<double>(
        "What is the yearly interest rate? ", ValidatePositiveFloat);
    ...
    const double first_months_interest = CalculateInterest(loan, interest_rate);
...
bool ValidatePositiveFloat(double to_check) { return to_check > 0.0f; }
